...
use std::sync::Arc;
  ...
  enum NodeKind {
    ...
    Head {
      ...
      _weak_pin: std::sync::Weak<dyn Any + Send + Sync>,
    },
  ...
  impl NodeKind {
    fn head(rc_pin: &Arc<dyn Any>) -> Self {
      let _weak_pin = Arc::downgrade(rc_pin);
      Self::Head { _weak_pin }
    }
...
mod tests {
  ...
  fn cancel_handle_pinning() {
    ...
    // succeed.
    assert!(Arc::get_mut(&mut cancel_handle).is_some());
    ...
    assert!(Arc::get_mut(&mut cancel_handle).is_none());
