...
impl<T: 'static> AsyncRefCell<T> {
  ...
  pub fn new_rc(value: T) -> Arc<Self> {
...
impl<T> AsyncRefCell<T> {
  pub fn borrow(self: &Arc<Self>) -> AsyncRefFuture<T> {
    AsyncRefFuture::new(self)
  }

  pub fn borrow_mut(self: &Arc<Self>) -> AsyncMutFuture<T> {
    AsyncMutFuture::new(self)
  }

  pub fn try_borrow(self: &Arc<Self>) -> Option<AsyncRef<T>> {
    Self::borrow_sync(self)
  }

  pub fn try_borrow_mut(self: &Arc<Self>) -> Option<AsyncMut<T>> {
    Self::borrow_sync(self)
  }
}
...
impl<T: 'static> RcRef<T> {
  ...
  pub(crate) fn split(rc_ref: &Self) -> (&T, &Arc<dyn Any>) {
...
impl<T: 'static> From<Arc<T>> for RcRef<T> {
...
impl<T: 'static> RcLike<T> for Arc<T> {}
...
mod tests {
  ...
  #[tokio::test]
  async fn rc_ref_map() {
    let three_cells = Arc::new(ThreeThings::default());
