...
pub trait Resource: Any + 'static {
  ...
  fn read(self: Rc<Self>, buf: Vec<u8>) -> AsyncResult<usize> {
    Box::pin(async move {
  ...
  fn write(self: Rc<Self>, _buf: Vec<u8>) -> AsyncResult<usize> {
    Box::pin(futures::future::err(not_supported()))
  }
...
impl dyn Resource + Send + Sync {
  ...
  /// dfgf
  pub fn downcast_rc<'a, T: Resource + Send + Sync>(self: &'a Rc<Self>) -> Option<&'a Rc<T>> {
    if self.is::<T>() {
...
pub struct ResourceTable {
  index: BTreeMap<ResourceId, Rc<dyn Resource + Send + Sync>>,
  next_rid: ResourceId,
}
...
impl ResourceTable {
  ...
  ///
  /// Returns a unique resource ID, which acts as a key for this resource.
  pub fn add_rc<T: Resource + Send + Sync>(&mut self, resource: Rc<T>) -> ResourceId {
    let resource = resource as Rc<dyn Resource + Send + Sync>;
  ...
  pub fn get<T: Resource + Send + Sync>(&self, rid: ResourceId) -> Result<Rc<T>, Error> {
    self
      .index
      .get(&rid)
  ...
  /// dfgdf
  pub fn get_any(&self, rid: ResourceId) -> Result<Rc<dyn Resource + Send + Sync>, Error> {
  ...
  pub fn replace<T: Resource + Send + Sync>(&mut self, rid: ResourceId, resource: T) {
    let result = self.index.insert(rid, Rc::new(resource) as Rc<dyn Resource + Send + Sync>);
  ...
  pub fn take<T: Resource + Send + Sync>(&mut self, rid: ResourceId) -> Result<Rc<T>, Error> {
  ...
  pub fn take_any(&mut self, rid: ResourceId) -> Result<Rc<dyn Resource + Send + Sync>, Error> {
...
#[derive(Default, )]
pub struct ResourceTable {
  index: BTreeMap<ResourceId, Arc<Mutex<dyn Resource + Send + Sync>>>,
  next_rid: ResourceId,
}
...
impl ResourceTable {
  ...
  /// Returns a unique resource ID, which acts as a key for this resource.
  pub fn add<T: Resource + Send + Sync>(&mut self, resource: T) -> ResourceId {
    self.add_rc(Arc::new(Mutex::new(resource)))
  }
  ...
  ///
  /// Returns a unique resource ID, which acts as a key for this resource.
  pub fn add_rc<T: Resource + Send + Sync>(&mut self, resource: Arc<Mutex<T>>) -> ResourceId {
    let resource = resource as Arc<Mutex<dyn Resource + Send + Sync>>;
    let rid = self.next_rid;
    let removed_resource = self.index.insert(rid, resource);
    assert!(removed_resource.is_none());
    self.next_rid += 1;
  ...
  /// given `rid`. If `rid` is not present or has a type different than `T`,
  /// this function returns `None`.
  pub fn get<T: Resource + Send + Sync>(&self, rid: ResourceId) -> Result<Arc<Mutex<T>>, Error> {
    self
      .index
      .get(&rid)
      .and_then(|rc| {
  ...
  /// dfgdf
  pub fn get_any(&self, rid: ResourceId) -> Result<Arc<Mutex<dyn Resource + Send + Sync>>, Error> {
    self.index.get(&rid).map(Clone::clone).ok_or_else(bad_resource_id)
  }
  ...
  /// Panics if the resource does not exist.
  pub fn replace<T: Resource + Send + Sync>(&mut self, rid: ResourceId, resource: T) {
    let result = self.index.insert(rid, Arc::new(Mutex::new(resource)) as Arc<Mutex<dyn Resource + Send + Sync>>);
    assert!(result.is_some());
  }
  ...
  /// it is not removed from the resource table. Note that the resource's
  /// `close()` method is *not* called.
  pub fn take<T: Resource + Send + Sync>(&mut self, rid: ResourceId) -> Result<Arc<Mutex<T>>, Error> {
    let resource = self.get::<T>(rid)?;
    self.index.remove(&rid);
    Ok(resource)
  }
  ...
  /// Removes a resource from the resource table and returns it. Note that the
  /// resource's `close()` method is *not* called.
  pub fn take_any(&mut self, rid: ResourceId) -> Result<Arc<Mutex<dyn Resource + Send + Sync>>, Error> {
    self.index.remove(&rid).ok_or_else(bad_resource_id)
  }
